...
class InfoClientRepository extends ServiceEntityRepository
{
    ...
    public function getclibyrole($id)
    {
        $entityManager = $this->getEntityManager()->getConnection();
        $query = 'Select * from info_client where id_adm = '.$id;
        $stmt = $entityManager->prepare($query);
        $rest = $stmt->executeQuery();
        return $rest->fetchAllAssociative();
    }
    public function ajout_1($id)
    {
        $entityManager = $this->getEntityManager()->getConnection();

        $query = 'UPDATE info_client set count_file = count_file + 1 where id ='.$id;
